...
export class PerformController {
  ...
  public async resolvePendingBlockingStateCalculationPerforms() {
    const blockingPerforms = this.pendingPerformList.filter(
      ({ perform }) => perform.blockingStateCalculation?.() ?? false,
    );
    if (blockingPerforms.length === 0) {
      return true;
    }
    if (blockingPerforms.some(({ perform }) => !perform.resolveStateCalculation)) {
      return false;
    }

    await Promise.all(blockingPerforms.map(({ perform }) => perform.resolveStateCalculation!()));
    return !this.hasPendingBlockingStateCalculationPerform();
  }
  /**
   * 是否存在正在运行且阻塞下一步的演出。
   *
