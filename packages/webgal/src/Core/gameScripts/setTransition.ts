...
export const setTransition = (sentence: ISentence): IPerform => {
  ...
  if (keepOffset) {
    ...
    if (currentTransform) {
      stageStateManager.updateAnimationSettings({ target: key, key: 'baseTransform', value: currentTransform });
    }
  }
  if (enterAnimation) {
  ...
  if (keepOffset !== null && keepOffset !== undefined) {
    stageStateManager.updateAnimationSettings({ target: key, key: 'enterKeepOffset', value: keepOffset });
    stageStateManager.updateAnimationSettings({ target: key, key: 'exitKeepOffset', value: keepOffset });
  }
  if (enterAnimation) {
